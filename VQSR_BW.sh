...
for vcf in $VCF_PATH/*.vcf; do
	...
	###FOR SNPS###
	echo "${JAVADIR}/java -Xmx10g -Djava.io.tmpdir=${SUBSAMPLE_DIR}/tmp -jar ${GATK_PATH}/GenomeAnalysisTK.jar -T VariantRecalibrator -R ${REF} -input $vcf -tranche 100.0 -tranche 99.9 -tranche 99.0 -tranche 90.0 -mode SNP -an QD -an MQ -an MQRankSum -an ReadPosRankSum -an FS -an SOR -an InbreedingCoeff -resource:hapmap,VCF,known=false,training=true,truth=true,prior=15.0 ${HAPMAP} -resource:omni,known=false,training=true,truth=true,prior=12.0 ${OMNI} -resource:1000G,known=false,training=true,truth=false,prior=10.0 ${G1000} -resource:dbsnp,known=true,training=false,truth=false,prior=2.0 ${DBSNP} -recalFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_recalibrate_SNPs -tranchesFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_tranches_SNPs -rscriptFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_VQSR_SNPs_only.plots.R --disable_auto_index_creation_and_locking_when_reading_rods --target_titv 3.2
	
	${JAVADIR}/java -Xmx10g -Djava.io.tmpdir=${SUBSAMPLE_DIR}/tmp -jar ${GATK_PATH}/GenomeAnalysisTK.jar -T ApplyRecalibration -R ${REF} -input $vcf -o ${OUT_DIR}/`basename ${vcf} .vcf`_VQSR_SNPs_only.vcf  -nt 5 -recalFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_recalibrate_SNPs -tranchesFile ${OUT_DIR}//VQSR_files/`basename ${vcf} .vcf`_tranches_SNPs --ts_filter_level ${SNP_SENSITIVITY} -mode SNP	

	###FOR INDELS###
	${JAVADIR}/java -Xmx10g -Djava.io.tmpdir=${SUBSAMPLE_DIR}/tmp -jar ${GATK_PATH}/GenomeAnalysisTK.jar -T VariantRecalibrator -R ${REF} -input ${OUT_DIR}/`basename ${vcf} .vcf`_VQSR_SNPs_only.vcf -tranche 100.0 -tranche 99.9 -tranche 99.0 -tranche 90.0 -mode INDEL -an QD -an MQRankSum -an ReadPosRankSum -an FS -an SOR -an InbreedingCoeff --maxGaussians 4 -resource:mills,known=false,training=true,truth=true,prior=12.0 ${MILLS} -resource:dbsnp,known=true,training=false,truth=false,prior=2.0 ${DBSNP} -recalFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_VQSR_SNPs_recalibrate_INDELs -tranchesFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_VQSR_SNPs_tranches_INDELs --disable_auto_index_creation_and_locking_when_reading_rods
	
	${JAVADIR}/java -Xmx10g -Djava.io.tmpdir=${SUBSAMPLE_DIR}/tmp -jar ${GATK_PATH}/GenomeAnalysisTK.jar -T ApplyRecalibration -R ${REF} -input ${OUT_DIR}/`basename ${vcf} .vcf`_VQSR_SNPs_only.vcf -o ${OUT_DIR}/`basename ${vcf} .vcf`_VQSR.vcf -recalFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_VQSR_SNPs_recalibrate_INDELs -tranchesFile ${OUT_DIR}/VQSR_files/`basename ${vcf} .vcf`_VQSR_SNPs_tranches_INDELs --ts_filter_level ${INDEL_SENSITIVITY} -mode INDEL" > ${SUBSAMPLE_DIR}/commands/VQSR/VQSR_`basename ${OUT_DIR}`/`basename ${vcf} .vcf`.sh 
